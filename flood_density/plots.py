...
def convert_kml_to_geojson(kml_file: str) -> str:
    gdf = gpd.read_file(kml_file, driver="KML")
    gdf.to_file(output_file, driver='GeoJSON')
    gdf.plot()
...
def city_bounds_and_density_plot(gdf: gpd.GeoDataFrame, centroides: np.ndarray, values_density: np.ndarray, bound: List[float]) -> fig:
